...
import {
  ...
  PlusCircle, Eye, List, Calendar, CreditCard,
} from 'lucide-react';
...
const StatCard: React.FC<StatCardProps> = ({
  label, value, icon: Icon, subtitle, trend = 'neutral', trendValue,
}) => {
      ...
      <div className="flex items-start justify-between" style={{ gap: '12px' }}>
        <div className="flex-1 min-w-0">
          <p style={{
            ...
            letterSpacing: '0.02em', textTransform: 'uppercase', margin: 0, marginBottom: '4px',
          }}>
            {label}
          </p>
          <p style={{
            fontSize: '22px', fontWeight: 700, color: 'var(--color-text-primary)',
            margin: 0, lineHeight: 1.2, letterSpacing: '-0.01em', fontVariantNumeric: 'tabular-nums',
          }}>
            {value}
          </p>
          {(subtitle || trendValue) && (
        ...
        <div
          ...
          style={{
            ...
            background: 'var(--color-bg-subtle)', color: 'var(--color-accent-text)',
          }}
        >
...
const QuickAction: React.FC<QuickActionProps> = ({ to, label, icon: Icon, description }) => (
  <Link
    ...
    style={{
      ...
      minHeight: '56px', flexDirection: 'row', alignItems: 'center',
  ...
  >
    <div
      className="flex items-center justify-center flex-shrink-0"
      style={{ width: '36px', height: '36px', borderRadius: '6px', background: 'var(--color-accent)', color: 'white' }}
    >
      <Icon style={{ width: '18px', height: '18px' }} />
    </div>
    <div className="flex-1 min-w-0">
      <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-text-primary)', margin: 0 }}>{label}</p>
      {description && (
...
const SectionHeader: React.FC<{
  ...
  linkTo?: string;
  linkText?: string;
}> = ({ icon: Icon, title, linkTo, linkText = 'View all' }) => (
  <div style={{
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    padding: '12px 20px 8px 20px', marginBottom: 0,
  }}>
    ...
    {linkTo && (
      <Link
        ...
        style={{
          ...
          fontSize: '11px', fontWeight: 500, color: 'var(--color-accent-text)',
...
export const DashboardHome: React.FC = () => {
  ...
  const filteredLabTransactions = useMemo(() => {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const lab = labTransactions || [];

    switch (dateFilter) {
      case 'today':
        return lab.filter((t) => new Date(t.createdAt) >= today);
      case 'week': {
        const weekStart = new Date(today);
        weekStart.setDate(today.getDate() - 7);
        return lab.filter((t) => new Date(t.createdAt) >= weekStart);
      }
      case 'month': {
        const monthStart = new Date(today);
        monthStart.setMonth(today.getMonth() - 1);
        return lab.filter((t) => new Date(t.createdAt) >= monthStart);
      }
      default:
        return lab;
    }
  ...
  const stats = useMemo(() => {
    ...
    const completedLabTests = filteredLabTransactions.filter((t) => t.status === 'completed').length;
    ...
    return {
      todaySales, yesterdaySales, salesTrend,
      lowStockCount, outOfStockCount,
      totalPurchaseOrders, pendingPOs, completedLabTests,
      totalProducts: products.length, totalTransactions, totalRevenue,
    };
  ...
  const dailySalesData = useMemo(() => {
    ...
    return days.map((day) => {
      const dayTransactions = filteredTransactions.filter((t) =>
        new Date(t.createdAt).toLocaleString('en', { weekday: 'short' }) === day
      );
      return {
        day,
    ...
    if (active && payload && payload.length) {
      return (
        <div style={{
          ...
          boxShadow: 'var(--shadow-lg)', background: 'var(--color-bg-elevated)',
          border: '1px solid var(--color-border)',
        }}>
          {label && (
            <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 2px 0' }}>
              {label}
            </p>
          )}
          {payload.map((p: any) => (
  ...
  // Role-based quick actions (attach icons)
  const quickActions = roleQuickActions.map((a) => ({
    to: a.to, label: a.label, description: a.description,
  ...
  return (
    <div className="w-full" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{
        ...
        flexWrap: 'wrap', gap: '12px', padding: '8px 4px',
      }}>
        <div>
          <h1 style={{
            fontSize: '20px', fontWeight: 700, color: 'var(--color-text-primary)',
            letterSpacing: '-0.01em', margin: 0, marginBottom: '4px',
          }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', flexShrink: 0 }}>
          <span style={{
            ...
            background: 'var(--color-bg-subtle)', fontSize: '11px',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          <DateFilterBtn label="Today" active={dateFilter === 'today'} onClick={() => setDateFilter('today')} />
          <DateFilterBtn label="Week" active={dateFilter === 'week'} onClick={() => setDateFilter('week')} />
          <DateFilterBtn label="Month" active={dateFilter === 'month'} onClick={() => setDateFilter('month')} />
          <DateFilterBtn label="All" active={dateFilter === 'all'} onClick={() => setDateFilter('all')} />
        </div>
        <span style={{
          fontSize: '10px', padding: '2px 10px', borderRadius: '9999px',
          background: 'var(--color-accent-light)', color: 'var(--color-accent-text)', fontWeight: 500,
        }}>
      ...
      <div style={{
        ...
        {statsVisible.todaySales && (
          <StatCard
            ...
            trendValue={stats.salesTrend !== 0
              ? `${stats.salesTrend > 0 ? '+' : ''}${stats.salesTrend.toFixed(1)}%`
              : undefined}
          />
        )}
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={dailySalesData}>
                    ...
                    <Area
                      ...
                      stroke="var(--color-accent)" fill="var(--color-accent-light)"
              ...
              <div style={{ padding: '4px 20px 16px 20px' }}>
                {paymentDistribution.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie
                        data={paymentDistribution} cx="50%" cy="50%"
                        innerRadius={45} outerRadius={65} paddingAngle={2} dataKey="value"
                      >
                      ...
                      {topProducts.map((product, index) => {
                        ...
                        return (
                          <div key={product.id} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span style={{ fontSize: '10px', fontWeight: 600, color: 'var(--color-text-muted)', width: '20px', textAlign: 'center' }}>
                              #{index + 1}
                            </span>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                                <span style={{
                                  fontSize: '11px', fontWeight: 500, color: 'var(--color-text-primary)',
                                  overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                                }}>
                                  {product.name}
                                </span>
                                <span style={{ fontSize: '10px', fontWeight: 600, color: 'var(--color-accent-text)', fontVariantNumeric: 'tabular-nums' }}>
                                  GHS {product.revenue.toFixed(2)}
                                </span>
                              </div>
                              <div style={{ height: '6px', borderRadius: '4px', background: 'var(--color-bg-subtle)', overflow: 'hidden' }}>
                                <div style={{
                            ...
                            <span style={{
                              fontSize: '9px', fontWeight: 500, color: 'var(--color-text-muted)',
                              width: '30px', textAlign: 'right', fontVariantNumeric: 'tabular-nums',
                            }}>
                              {product.quantity}
                            </span>
                          </div>
        ...
        {quickActions.length > 0 && (
          <div className="lg:col-span-2 card" style={{ padding: 0, overflow: 'hidden', background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: '10px' }}>
            ...
            <div style={{
              ...
              gap: '10px', padding: '4px 20px 20px 20px',
            }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2" style={{ gap: '12px' }}>
          {recentVisible.transactions && (
            <div className="card" style={{ padding: 0, overflow: 'hidden', background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: '10px' }}>
              ...
              <div style={{ padding: '4px 20px 20px 20px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {recentTransactions.length === 0 ? (
                  <div className="text-center" style={{ padding: '16px 0' }}>
                    <div className="flex items-center justify-center mx-auto" style={{ width: 36, height: 36, borderRadius: '9999px', background: 'var(--color-bg-subtle)', marginBottom: '8px' }}>
                  ...
                  recentTransactions.map((tx) => (
                    <div
                      ...
                      style={{
                        ...
                        background: 'var(--color-bg-subtle)', transition: 'background 150ms ease',
                    ...
                    >
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--color-text-secondary)', letterSpacing: '-0.01em', margin: 0 }}>
                      ...
                      <div className="text-right flex-shrink-0" style={{ marginLeft: '8px' }}>
                        <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--color-text-primary)', margin: 0, marginBottom: '2px', fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden', background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: '10px' }}>
              ...
              <div style={{ padding: '4px 20px 20px 20px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {recentVisible.labTests && recentLabTests.length > 0 && (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 6px', marginBottom: '2px' }}>
                    ...
                    {recentPurchaseOrders.map((po) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p style={{ fontSize: '11px', color: 'var(--color-text-muted)', margin: 0, marginTop: '2px' }}>
                            {(po as any).supplierName || (po as any).supplier?.name || 'Supplier'}
                          </p>
                        </div>
                        <div className="text-right flex-shrink-0" style={{ marginLeft: '8px' }}>
